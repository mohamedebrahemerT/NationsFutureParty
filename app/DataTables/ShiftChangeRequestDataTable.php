...
class ShiftChangeRequestDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('shift_name', function ($row) {
                return $row->shiftSchedule->shift->shift_name;
            })
            ->editColumn('shift', function ($row) {
                return '<span class="badge badge-info" style="background-color: '.$row->shift->color.'">'.$row->shift->shift_name.'</span>';
            })
    ...
    public function query(EmployeeShiftChangeRequest $model)
    {
        ...
        if (!is_null($shift) && $shift !== 'all') {
            $model->where('employee_shift_change_requests.shift_schedule_id', '=', $shift);
        }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.attendance.shift') => ['data' => 'shift', 'name' => 'shift_name', 'title' => __('modules.attendance.shift')],
