...
class RolePermissionController extends AccountBaseController
{
    ...
    {
        Role::where('id', $id)->update(['display_name' => ucwords($request->role_name), 'name' => $request->role_name]);
    }
    ...
    public function addMissingAdminPermission()
    {
        ...
        if ($adminRole) {
            ...
            $this->addMissingUserPermission($adminRole->id, $allTypePermisison->id);
        }
    }
    ...
    public function addMissingEmployeePermission()
    {
        ...
        foreach ($roles as $employeeRole) {
            ...
            $this->addMissingUserPermission($employeeRole->id, $noneTypePermisison->id);
        }
    }

    public function addMissingUserPermission($roleId, $permissionTypeId)
    {
        ...
        foreach ($users as $user) {
            $employeePermission = UserPermission::where('user_id', $user->id)->pluck('permission_id')->toArray();
                    
            if ($role->permissions_count != count($employeePermission)) {
