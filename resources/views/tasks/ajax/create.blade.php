        ...
        <x-form id="save-task-data-form">
            ...
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                            ...
                            <select class="form-control select-picker" name="category_id" id="task_category_id"
                                ...
                                @if ($viewTaskCategoryPermission == 'all' || $viewTaskCategoryPermission == 'added')
                                    @foreach ($categories as $category)
                                        <option @if (!is_null($task) && $task->task_category_id == $category->id) selected @endif value="{{ $category->id }}">{{ ucwords($category->category_name) }}
                                        </option>
                    ...
                    <div class="col-md-5 col-lg-4 dueDateBox"  @if($task && is_null($task->due_date)) style="display: none" @endif>
                            ...
                            <div class="col-md-12 col-lg-4">
                                <x-forms.select fieldName="milestone_id" fieldId="milestone_id"
                                    ...
                                    @foreach ($milestones as $item)
                                        <option value="{{ $item->id }}" @if (!is_null($task) && $item->id == $task->milestone_id) selected @endif>{{ $item->milestone_title }}</option>
                                    @endforeach
                                </x-forms.select>
                            </div>
