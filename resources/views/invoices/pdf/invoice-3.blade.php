...
<div id="container">
    ...
    <section id="invoice-info">
        <div>
            <span>@lang('modules.invoices.invoiceDate'):</span> <span>{{ $invoice->issue_date->format(global_setting()->date_format) }}</span>
        </div>
        @if (empty($invoice->order_id) && $invoice->status === 'unpaid' && $invoice->due_date->year > 1)
        <div>
            <span>@lang('app.dueDate'):</span> <span>{{ $invoice->due_date->format(global_setting()->date_format) }}</span>
        </div>
        @endif
        <div>
